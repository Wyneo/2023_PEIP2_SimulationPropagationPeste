...
WIN = pygame.display.set_mode((int(hauteur),int(hauteur)))  # fenêtre carré
...
# [[(centreisitemx,centreisitemy),rayonisitem],[C6],[Parking],[E5],[75]] centre et rayon des cercles, 0 pour les rectangles afin d'avoir autant d'indice que Verif et nbsurface-1
...
chemin = [[[- 3.53,1066.32 * fact2],[-3.46,1111.66 * fact2]],
          ...
          [[-23,8249 * fact2],[-25,9355 * fact2]]] #[[[chemin1 xmin a,b],[chemin1 xmax a,b]],[]]
# Utilisé dans VerifPos2, inutilisé pour l'instant
...
# [[[chemin1 fxmin],[chemin1 fxmax]],]
# Points répertoriés par surface(11 chemins puis 3 batiments), d'abord ceux correspondant à fxmin puis ceux fxmax
...
# depart = [[300,450] if i < 100 else [400,290] if 100 <= i < 200 else [350,130] if 200 <= i < 300 else [centreisitem[0],centreisitem[1]] for i in range(nbtot)]
...
# verifru = False  # True si les points sont allé dans le ru UTILE ???
...
for i in range(nbtot):
    ...
    #cercle = habitant('white',250,250, test, rayon_cercle, WIN, endroit1, preendroit1)
...
"""
# position des rats dans 3 chemins différend
for k1 in range(nbvert + nbjaune, nbvert + nbjaune + int(nbrat/3)):  # positionnement des premiers rats
    Habitant, x4, y4 = DéplacementPoints2('gray', 238,270, val_aleatoire, rayon_cercle, k1,test,WIN,Habitant,infocercles,fact2,p,nbsurface,Verif)
for k2 in range(nbvert + nbjaune + int(nbrat/3),nbvert + nbjaune + int(2*nbrat/3)):  # positionnement des autres rats
    Habitant, x4, y4 = DéplacementPoints2('gray', 470,217, val_aleatoire, rayon_cercle, k2, test, WIN, Habitant, infocercles,fact2,p,nbsurface,Verif)
for k3 in range(nbvert + nbjaune + int(2*nbrat/3),nbvert + nbjaune + nbrat):  # positionnement des autres rats
    Habitant, x4, y4 = DéplacementPoints2('gray', 300,177, val_aleatoire, rayon_cercle, k3, test, WIN, Habitant, infocercles,fact2,p,nbsurface,Verif)
"""
...
while continuer:  # boucle infinie
    ...
    if simulation_active:  # si on veut que la simul continue
        ...
        for i in range(nbtot):
            ...
            if n2 * tattRUinf < test < (n2 * tattRUinf) + tattRUdedans:  # Si on est au milieux de la journée (entre 12H-14h)
                autorisation[12] = 1/2*nbtot  # On autorise la moitié des gens dans le RU
                pourcent = random.uniform(0, 1)  # On tire un chifffre aléatoire
                if pourcent < 0.1 and not(Habitant[i].ru) and Habitant[i].ru != 'black' and Habitant[i].color != 'gray':  # Les habs ont 0.1% de chance d'aller dans le rue, s'ils y son tpas déjà et s'ils sont pas morts ou des rats
            ...
            if -1 < Habitant[i].endroit < 14 and Habitant[i].color != 'gray':  # si pas nul part et pas dans un cercle
                Habitant[i].posx, Habitant[i].posy = collmur(Habitant, i, fact2, points, fxmax, fxmin, fymax, fymin, p,stop)  # appel la fonction collmur
            if 14 <= Habitant[i].endroit < 19 and Habitant[i].color != 'gray':  # si dans iun cercle
    ...
    if test == n3 * int(nbTBPJ*9):  # s'il y a eu le nombre de tour de boucle pour un jour
        n3 += 1  # jour suivant
        #print('jour : ', n3)
...
for i in range(nbtot):
    if couleur[i] == 'r':
        sommeR0 += R0[i]
        nbrougetot += 1
print("nb de tour de boucle : ", test)
...
# verifru UTILE ?????  FAIT
# fonction attribut position inutile ??? FAIT
# trop d'argument à DéplacemrntsPoitns2  FAIT
# changer nom de dansisitem

# manque chg chemin E5    FAIT
# ajouter les noms des bats

# ROUGE contaminent trop les verts