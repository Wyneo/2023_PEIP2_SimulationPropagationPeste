...
from math import *
import random
from classe_modelisation1 import *
...
entrées : couleur voulu, positions x et y du point, le temps à l'instant ou les points sont créé, déplacements x et y, rayon du point, numéro i du point
"""
...
entrées : couleur1,couleur2,nombre d'habitants de la couleur c1 et c2, nb d'habitants total, rayon_cercle, facteur de réussite de la collision, nb de tours
...
def verifTempsSup(c1,c2,nb1,nb2,tattente,facteur,i,test):
    if Habitant[i].color == c1:
        ...
        if test - Habitant[i].temps > tattente and pourcent < facteur:  # si le temps est supérieur au temps d'attente et que le nb aleatoire est inférieur au facteur définit
            ...
            nb1 -= 1
            nb2 += 1
            if c2 == 'black':  # Habitant mort envoyé très loin
...
def rat(c1,c2,nb1,nb2,test,tattente,n,i):
    if Habitant[i].color == c1:
        if test - Habitant[i].temps > n*tattente:
            ...
            nb1 -= 1
            nb2 += 1
            Habitant[i].posx, Habitant[i].posy = -10000, -10000 # Habitant mort envoyé très loin
    return n,nb1,nb2
...
while continuer:  # boucle infinie
    for event in pygame.event.get():
        if event.type == pyl.QUIT:  # quiter le prog et que ça fasse pas une boucle infinie
            continuer = False
    if nbrat == 0:  # arrêt quand il n'y a plus de rats
        continuer = False
    if simulation_active:  # si on veut que la simul continue
        screen.fill('black')  # couleur du fond
        font = pygame.font.Font(None, int(screen_size_x/50))
        time1 = pygame.time.get_ticks() # temps de départ
        time2 = int(time1/ 1000) # transforme time1 en seconde
        text = font.render('Seconde : ' + str(time2), True, 'white')  # transforme en texte time2
        nbboucle = font.render('Tour de boucle : ' + str(test), True,'white')  # transforme en texte test, correspond au nombre de boucle
        for i in range(nbtot):
            x = random.uniform(-val_aleatoire,val_aleatoire)  # génère une valeur aléatoire (correspondra au déplacement en x) entre -val_aleatoire et val_aleatoire
            y = random.uniform(-val_aleatoire,val_aleatoire)  # génère une valeur aléatoire (correspondra au déplacement en y) entre -val_aleatoire et val_aleatoire
        ...
        screen.blit(text, (10, 10))  # affiche le texte sur le fond
        screen.blit(nbboucle, (160, 10))  # affiche le nombre de tours
...
# affiche le nombre de tours de boucle, de vert restant, de contamination, de rouge qu'il y a eu au total et le R0



# ajout dans un tableur des nouvelles stats, crée automatiquement le tableur si pas déjà existant
with open("stat 1ère modélisation.csv", "a") as fichier:
    fichier.write("\n")
    fichier.write("\t")
    fichier.write("\t")
