...
def calculprop(Habitant,i,prop,Verif,fact2,p,infocercles,rayon_cercle):
    if -1 < Habitant[i].endroit < 14 and Habitant[i].color != GRIS:  # on compte pas les rats, pas besoin d'exclure les morts ils sont partis loins
        if Verif[Habitant[i].endroit](Habitant[i].posx,Habitant[i].posy,fact2,p):
            prop[Habitant[i].endroit] += 1
    if 14 <= Habitant[i].endroit < 19 and Habitant[i].color != GRIS:  # on compte pas les rats, pas besoin d'exclure les morts ils sont partis loins
...
def verifprop(Habitant,prop,autorisation,stop,nbtot,nbsurface,fact2, points, fxmax, fxmin, fymax, fymin, p,infocercles,Verif,rayon_cercle):
    ...
    for i in range(nbtot):
        ...
        if stop[Habitant[i].endroit] and Habitant[i].color != GRIS:  # si trop d'hab, sauf si c'est un rat
