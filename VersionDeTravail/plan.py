...
def draw_window(x,y):
    ...
    for i in range(len(points) - 3):
        pygame.draw.polygon(WIN, GRAY, (points[i][0], points[i][1], points[i][2], points[i][3]))
    pygame.draw.polygon(WIN, GRAY, (p[18], p[25], (250 * fact2, 508 * fact2), (252 * fact2, 475 * fact2)))
    pygame.draw.polygon(WIN, GRAY, (p[0], p[13], (219 * fact2, 377 * fact2), (188 * fact2, 385 * fact2)))
    pygame.draw.polygon(WIN, GRAY, (p[3], p[4], (377 * fact2, 130 * fact2), (352 * fact2, 130 * fact2)))
    ...
    for i in range(len(p)-1):                                                          # chemins
        if i!=3 and i!=6 and i!=9 and i!=13 and i!=15 and i!= 17 and i!=20 and i!=23 and i!=25 and i!=27 and i!=29:
            pygame.draw.line(WIN,BLACK,p[i],p[i+1],1)
...
for i in range(nbvert):
    Habitant, x1, y1 = DéplacementPoints2(VERT, Habitant[i].posx, Habitant[i].posy, val_aleatoire, rayon_cercle, i, test,WIN,Habitant,infocercles,fact2,p,nbsurface,Verif)  # appel la fonction DéplacementPoints
for j in range(nbvert, nbvert + nbjaune):
    Habitant, x3, y3 = DéplacementPoints2(JAUNE, Habitant[j].posx, Habitant[j].posy, val_aleatoire, rayon_cercle, j, test,WIN,Habitant,infocercles,fact2,p,nbsurface,Verif)
for k in range(nbvert + nbjaune, nbvert + nbjaune + nbrat):
    Habitant, x4, y4 = DéplacementPoints2(GRIS, hauteur/2,hauteur/2, val_aleatoire, rayon_cercle, k,test,WIN,Habitant,infocercles,fact2,p,nbsurface,Verif)
    ...
    if simulation_active:  # si on veut que la simul continue
        ...
        for i in range(nbtot):
            ...
            if n2 * tattRUinf < test < (n2 * tattRUinf) + tattRUdedans:  # Si on est au milieux de la journée (entre 12H-14h)
                ...
                if pourcent < factru and not(Habitant[i].ru) and Habitant[i].color != 'black' and Habitant[i].color != GRIS:  # Les habs ont 0.1% de chance d'aller dans le RU, s'ils y sont pas déjà et s'ils sont pas morts ou des rats
            ...
            if test == n3*int(nbTBPJ):  # nouveau jour
                if Habitant[i].color != 'black' and Habitant[i].color != GRIS:  # si les habs sont pas morts ou des rats
            ...
            if -1 < Habitant[i].endroit < 14 and Habitant[i].color != GRIS:  # si pas nul part et pas dans un cercle et pas un rats
                Habitant[i].posx, Habitant[i].posy = collmur(Habitant, i, fact2, points, fxmax, fxmin, fymax, fymin, p,stop)  # appel la fonction collmur
            if 14 <= Habitant[i].endroit < 19 and Habitant[i].color != GRIS:  # si dans iun cercle et pas un rats
                Habitant[i].posx, Habitant[i].posy = dansisitem(Habitant, i, infocercles,stop)  # appel la fonction dans isitem (gère les cercles)
            if Habitant[i].color == GRIS: # empèche les rats de sortir de la map
            ...
            nbjaune, nbrouge = verifTempsSup(JAUNE, 'red', nbjaune, nbrouge, tattjr, factjr, i,test)  # les points jaunes deviennent rouges au bout d'un moment
            ...
            nbjaune, nbvert = verifTempsInf(JAUNE, VERT, nbjaune, nbvert, tattjv, factjv, i,test)  # les points jaunes deviennent verts (guérissent) à un temps donné
            nbjaune, nbmort = verifTempsSup(JAUNE, 'black', nbjaune, nbmort, tattjn, factjn, i,test)  # les points jaunes deviennent noirs (meurent) au bout d'un moment
            n, nbrat, nbmort = rat(GRIS, 'black', nbrat, nbmort, test, tattgn, n,i)  # les points gris (rats) deviennent noirs (meurent) au bout d'un moment
            nbrouge, nbvert = verifTempsInf('red', VERT, nbrouge, nbvert, tattrv, factrv, i,test)  # les points rouges deviennent verts (guérissent) à un temps donné
        ...
        nbvert, nbrouge = collision(VERT, 'red', nbvert, nbrouge, nbtot, rayon_cercle, factvr,test)  # appel la fonction collision : les points verts sont contaminés par les rouges
        nbvert, nbjaune = collision2(GRIS, VERT, JAUNE, nbrat, nbvert, nbjaune, nbtot, rayon_cercle, factgvj,test)  # appel la fonction collision2 : les points gris (rats) contaminent les verts  qui deviennent jaunes
